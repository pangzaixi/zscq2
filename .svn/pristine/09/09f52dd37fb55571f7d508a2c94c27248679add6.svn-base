...
@Controller("stockUseController")
@Scope("prototype")
public class StockUseController extends GenericForwardComposer{
	...
	int index = 0;//列表当前选定项
	public Listbox stockUseListbox;//页面遍历方式加载数据
	 ...
	 */
	public void onClick$saveButton() throws ParseException{
		
		 User user = (User)session.getAttribute("user");
		if(stockUseListbox.getSelectedItem() != null){
			if (car.getSelectedItem() != null || (driver.getValue() != null && !driver.getValue().trim().equals(""))) {
				Stock stock1 = (Stock)stockUseListbox.getSelectedItem().getValue();
				Map<Object, Object> stockMap = new HashMap<Object, Object>();
				stockMap.put("pjid", stock1.getPjid());
				stockMap.put("companyId", user.getCompanyid());
				synchronized(this) {
					Stock stock2 = stockService.getByPj(stockMap);
					if (stock2.getTotalNum() >= totalNum.getValue().doubleValue()) {
						Double d = stock2.getTotalNum() - totalNum.getValue().doubleValue();
						for(Stock tmp:list_stock){
							if(tmp.getId() == stock2.getId()){
								Map<Object,Object> map = new HashMap<Object,Object>();//
								addStockUse(stock2,map);//保存盘库记录
								map.clear();
								if (totalNum.getValue().doubleValue() != 0) {
									map.put("id", tmp.getId());
									map.put("totalNum",d);
									map.put("amounts",stock2.getAmounts() - totalNum.getValue().doubleValue() * stock2.getAgeragePrice());
									if (d == 0) {
										map.put("ageragePrice",stock2.getAgeragePrice());
										map.put("amounts",0);
										tmp.setAgeragePrice(0);
										tmp.setAmounts(0);
									}else {
										map.put("ageragePrice",stock2.getAgeragePrice());
										map.put("amounts",stock2.getAmounts() - totalNum.getValue().doubleValue() * stock2.getAgeragePrice());
										tmp.setAgeragePrice(stock2.getAgeragePrice());
										tmp.setAmounts(stock2.getAmounts() - totalNum.getValue().doubleValue() * stock2.getAgeragePrice());
									}
						...
						Messagebox.show("领用成功，领取" + pjName.getValue() + totalNum.getValue() + pjdw.getValue() +
								";" + "库存剩余" + d,"注意", Messagebox.OK, Messagebox.EXCLAMATION);
						listModelList = new ListModelList(list_stock);
						stockUseListbox.setModel(listModelList);//刷新数据后会失去焦点,所以不能在这个方法里指定当前选定项,需要在onBlur失去焦点方法里重新指定当前选定项
	 ...
	 * @return
	 */
	private void addStockUse(Stock stock,Map<Object,Object> map){
		User user = (User)session.getAttribute("user");
		map.put("creator", user.getUser_name());
		map.put("amountsDz",stock.getAgeragePrice() * totalNum.getValue().doubleValue());
	...
	public void onCreate$stockUseListbox(){
		System.out.println();
	}
	/**
	 * 失去焦点方法
	 */
	public void onBlur$stockUseListbox(){
		//stockUseListbox.setSelectedIndex(2);
	}

	/**
	 * 列表单击事件
	 */
	public void onClick$stockUseListbox(){
		if(stockUseListbox.getSelectedItem() != null){
			Stock stock = (Stock)stockUseListbox.getSelectedItem().getValue();
			totalNum.setValue(BigDecimal.ZERO);//数量
			amounts.setValue(BigDecimal.ZERO);//金额
	 ...
	 public void onClick$clearButton(){
		...
		listModelList = new ListModelList(list_stock);
		stockUseListbox.setModel(listModelList);
		erweima.focus();
	    erweima.setValue("");
	 }
