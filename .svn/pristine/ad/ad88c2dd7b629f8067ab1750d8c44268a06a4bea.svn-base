 ...
 *
 */
public class TableForSbbhfssq {

public void sbbhfs(XWPFDocument document,ZdBean zdBean,List<ZdDetail> list,WfCompany wfCompanyBean,boolean zJexport){
	//基本信息表格
...
//    infoTable.getCTTbl().getTblPr().unsetTblBorders();
    ...
    for(int i=0;i<list.size();i++){
    	...
    	if(zJexport){run6.setText(String.valueOf(df.format(list.get(i).getGf())));}else{run6.setText(String.valueOf(df.format(list.get(i).getGf_yt())));}
    	XWPFRun run7 = infoTableRowTwo.getCell(7).getParagraphs().get(0).createRun();run7.setFontFamily("仿宋");
    	if(zJexport){run7.setText(String.valueOf(df.format(list.get(i).getDlf())));}else{run7.setText(String.valueOf(df.format(list.get(i).getDlfYt())));}
    ...
    if(zJexport){
    	infoTableRowLast1.getCell(6).setText(String.valueOf(df.format(zdBean.getSumgf())));
    	infoTableRowLast1.getCell(7).setText(String.valueOf(df.format(zdBean.getSumdlf())));
    }else{
    	infoTableRowLast1.getCell(6).setText(String.valueOf(df.format(zdBean.getSumgfYt())));
    	infoTableRowLast1.getCell(7).setText(String.valueOf(df.format(zdBean.getSumdlfYt())));
    }
    ...
    if(zJexport){
    	infoTableRowLast2.getCell(6).setText(String.valueOf(df.format(zdBean.getSumgf()+zdBean.getSumdlf())));
    }else{
    	infoTableRowLast2.getCell(6).setText(String.valueOf(df.format(zdBean.getSumgfYt()+zdBean.getSumdlfYt())));
    }
    infoTableRowLast2.getCell(6).getCTTc().addNewTcPr().addNewHMerge().setVal(STMerge.RESTART);
    infoTableRowLast2.getCell(7).getCTTc().addNewTcPr().addNewHMerge().setVal(STMerge.CONTINUE);
