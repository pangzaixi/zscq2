...
boolean zJexport = true;//导出类型，字节导出true，还是盈天导出false
List<WfCompany> list_wf  = new ArrayList<WfCompany>();
//获取service对象
...
public void doAfterCompose(Component comp) throws Exception {
	...
	//我方公司主体
    list_wf = zdService.selectWfCompany();
...
public void onClick$queryButton(){
	...
	if(list != null && list.size()>0){
		...
		sumJe.setValue("官费小计："+count1.get("a")+"元   代理费小计："+count1.get("b")+"元    总计： "+count1.get("c")+"元 ，盈天代理费合计："+count1.get("d")+"元");
	}
...
public void onClick$exportButton() throws IOException{
	...
	if("异议申请".equals(zdBean.getAjlx()) || "撤三申请".equals(zdBean.getAjlx()) || "撤三复审申请".equals(zdBean.getAjlx()) ||
		...
		yysq.yysq(zdBean,list,wfCompanyBean,zJexport);
	}else{
...
public void onClick$ytExportButton() throws IOException{
	...
	if(wfCompany.getSelectedItem() == null ){
		...
		String a = wfCompany.getSelectedItem().getValue();
		for(WfCompany t:list_wf){
			if(a.equals(t.getWfName())){
				wfCompanyBean =t;
				break;
			}
		}
