...
@Controller("downLoadFileControllerYmiao")   
@Scope("prototype") 
public class DownLoadFileControllerYmiao extends GenericForwardComposer{
...
//		return;
	...
	list.clear();
	 ...
	 path = path.substring(0, path.indexOf("WEB-INF"))+"/upload/yumiao/";
	 List<File> fileList = new ArrayList<File>();
	 
	 getFiles(path,fileList);
	
	 if(fileNums>2000){
		 Messagebox.show("查询结果超过2000条，显示前2000条");
	 }
	 if(fileList.size()>0){
...
//				lazb_t.setSbh(lazb.getSbh());
				lazb_t.setRemarks(file.getPath().substring(file.getPath().indexOf("zscq2\\upload\\yumiao\\")+15));
				list.add(lazb_t);
